...
class LoginController extends Controller {
    ...
    async login() {
        ...
        await this.ctx.render('index/login/login.html',{
            returnUrl:returnUrl
        });
    }
    ...
    async doLogin() {
      ...
      if (identify_code.toUpperCase()!= this.ctx.session.identify_code.toUpperCase()) {
          ...
          // cookies 安全      加密
          this.service.cookie.set('userinfo', userResult[0]);
          this.ctx.body = {
              success: true,
              msg: '登录成功',
          };
    ...
    // 注册第一步 输入手机号
    async registerStep1() {
        await this.ctx.render('index/login/register_step1.html');
    }
    ...
    async registerStep2() {
      ...
      if (userTempResult.length == 0) {
          this.ctx.redirect('/register/step1');
      } else {
          await this.ctx.render('index/login/register_step2.html', {
            sign,
    ...
    async registerStep3() {
      ...
      if (userTempResult.length == 0) {
          ...
          await this.ctx.render('index/login/register_step3.html', {
            sign,phone_code,msg,
          });
      }
    }
    ...
    async doRegister() {
      ...
      if (userTempResult.length == 0) {
        ...
        if (password.length < 6 || password != rpassword) {
          ...
          this.ctx.redirect('/register/step3?sign=' + sign + '&phone_code=' + phone_code + '&msg=' + msg);
        } else {
          const userModel = new this.ctx.model.User({
            phone: userTempResult[0].phone,
            password: await this.service.tools.md5(password),
            last_ip: ip,
            name:userTempResult[0].phone,
            nicheng:userTempResult[0].phone,
          });
          // 保存用户
          ...
          if (userReuslt) {
            ...
            // 用户注册成功以后默认登录
            // cookies 安全      加密
            this.service.cookie.set('userinfo', userinfo[0]);
            this.ctx.redirect('/');
          }
        }
      }
        ...
        if (!reg.test(phone)) {
          ...
          if (userTempResult.length > 0) {
            if (userTempResult[0].send_count < 6 && ipCount < 10) { // 执行发送
              ...
              console.log('---------------------------------');
              console.log("同一ip下第"+ipCount+"个号注册");
              console.log('---------------------------------');
              console.log("同一号下第"+userTempResult[0].send_count+"次发送验证码");
